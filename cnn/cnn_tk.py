...
class app:
    ...
    def model(self,x):
        ...
        for i in range(struct.shape[1] - 1):
            ...
            net = tf.layers.dropout(net, rate=self.phDropRate)

        net = tf.layers.dense(inputs=net, units=self.nOutputs, activation=outputs_af, kernel_regularizer=self.regularizer,
                              bias_regularizer=self.regularizer, kernel_initializer=self.initializer,
                              bias_initializer=self.initializer)
    ...
    def initModel(self):
        ...
        self.x, self.y = tf.placeholder(tf.float32, shape=[None, self.nInputs]), tf.placeholder(tf.float32, shape=[None, self.nOutputs])
        self.dataset = tf.data.Dataset.from_tensor_slices((self.x, self.y)).batch(self.phBatchSize).shuffle(1).repeat()
        ...
        self.train_op = tf.train.AdamOptimizer(learning_rate=self.phLearningRate).minimize(self.loss_reg)
        ...
        self.model.add(Conv1D(kernel_size=10, filters=20, activation='relu', input_shape=(self.nInputs, 1), padding="same",
    ...
    def thread_train(self):
        ...
        self.sess.run(self.iter.initializer, feed_dict={self.x: self.X_train, self.y: self.Y_train, self.phBatchSize: self.batchsize, self.phDropRate: self.settings['drop_rate'],
        ...
        self.sess.run(self.iter.initializer,
                      feed_dict={self.x: self.X_train, self.y: self.Y_train, self.phBatchSize: self.nTrainSize,
                                 self.phDropRate: 0,
        ...
        if (TestSizePercent > 0.0):
            self.sess.run(self.iter.initializer,
                          feed_dict={self.x: self.X_test, self.y: self.Y_test, self.phBatchSize: self.nTestSize,
                                     self.phDropRate: 0,
                                     self.batch_normalization_active: False})
        ...
        for i in range(self.settings['epochs']):
            self.sess.run(self.iter.initializer,
                     feed_dict={self.x: self.X_train, self.y: self.Y_train, self.phBatchSize: self.batchsize, self.phDropRate: self.settings['drop_rate'],
                                self.batch_normalization_active: True})
            train_loss = 0
            for _ in range(self.n_batches_train):
                _, loss_value = self.sess.run([self.train_op, self.loss], feed_dict={self.phLearningRate: self.settings['ls']})
                train_loss += loss_value
            train_loss /= self.n_batches_train
            self.trainingdata_train_error = np.append(self.trainingdata_train_error, train_loss)
            if TestSizePercent > 0.0:
                self.sess.run(self.iter.initializer, feed_dict={self.x: self.X_test, self.y: self.Y_test, self.phBatchSize: self.nTestSize, self.phDropRate: 0,
                                                                self.batch_normalization_active: True})
            ...
            if TestSizePercent>0.0:
                if (test_loss < p_test_loss and i > 0 and train_loss < p_train_loss):
                    ...
                    if (self.test_model == True):
                        ...
                        self.sess.run(self.iter.initializer,
                                      feed_dict={self.x: self.X, self.y: self.Y, self.phBatchSize: self.n_datasize,
                                                 self.phDropRate: 0,
                                                 self.batch_normalization_active: False})
        ...
        self.sess.run(self.iter.initializer,
                      feed_dict={self.x: self.X, self.y: self.Y, self.phBatchSize: self.n_datasize, self.phDropRate: 0, self.batch_normalization_active: False})
        ...
        while True:
            if (os.path.isfile(self.sDataInput1Path)):
                ...
                try:
                    ...
                    self.sess.run(self.iter.initializer,
                                  feed_dict={self.x: X0, self.y: Y0, self.phBatchSize: 1, self.phDropRate: 0, self.batch_normalization_active: False})
    ...
    def theadDrawTestingPlot(self, i):
        ...
        if(self.data_is_loading==False):
            if(self.training_is_launched==False):
                self.sess.run(self.iter.initializer,
                              feed_dict={self.x: self.X, self.y: self.Y, self.phBatchSize: self.n_datasize, self.phDropRate: 0,
                                         self.batch_normalization_active: False})
    ...
    def loadData(self):
        ...
        self.X = np.reshape(self.X, [self.n_datasize, self.nInputs])
        self.Y = np.reshape(self.Y, [self.n_datasize, self.nOutputs])
